...
####################################################################
## (new) function to compute DiaBLE universe size
####################################################################

def calculate_universe_size_diable(G, cluster_nodes, not_in_cluster):
    """
    Calculate the size of the universe, which includes:
    - The current disease module (cluster_nodes).
    - The candidate genes (not_in_cluster).
    - The first neighbors of the candidate genes.
    """
    universe_nodes = set(cluster_nodes)
    for node in not_in_cluster:
        universe_nodes.add(node)
        universe_nodes.update(G.neighbors(node))
    return len(universe_nodes)
...
def diamond_iteration_of_first_X_nodes(G, S, X, alpha):
    ...
    while len(added_nodes) < X:
        ...
        ###################################################################
        # Change the Diable Universe Size
        diable_N = calculate_universe_size_diable(G, cluster_nodes, not_in_cluster)
        ###################################################################
        ...
        for node, kbk in reduced_not_in_cluster.items():
            ...
            try:
                ...
                p = pvalue(kb, k, diable_N, s0, gamma_ln)
                all_p[(k, kb, s0)] = p
